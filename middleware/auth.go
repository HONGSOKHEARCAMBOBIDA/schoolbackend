...
import (
	...
	"schoolbackend/utils" // Custom utilities package (contains JWT secret key)
...
func AuthMiddleware() gin.HandlerFunc {
	// or we can write fun(*gin.context)
	...
	return func(c *gin.Context) {
		...
		token, err := jwt.Parse(tokenString, func(token *jwt.Token) (interface{}, error) { 
			// jwt.Parse decode និងយក header និង claims ទៅជា Go struct (*jwt.Token).
			//​jwt.parse decode JWT
